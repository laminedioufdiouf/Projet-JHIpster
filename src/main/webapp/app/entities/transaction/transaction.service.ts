...
export class TransactionService {
  ...
  protected convertDateFromClient(transaction: ITransaction): ITransaction {
    const copy: ITransaction = Object.assign({}, transaction, {
      dateexpediteur:
        transaction.dateexpediteur != null && transaction.dateexpediteur.isValid() ? transaction.dateexpediteur.toJSON() : null,
      dateretrait: transaction.dateretrait != null && transaction.dateretrait.isValid() ? transaction.dateretrait.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateexpediteur = res.body.dateexpediteur != null ? moment(res.body.dateexpediteur) : null;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((transaction: ITransaction) => {
        transaction.dateexpediteur = transaction.dateexpediteur != null ? moment(transaction.dateexpediteur) : null;
